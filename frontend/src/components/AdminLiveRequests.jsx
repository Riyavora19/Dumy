...
const AdminLiveRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/live-requests', { params });
    ...
    try {
      if (editingRequest) {
        const response = await axios.put(
          `http://localhost:5000/api/live-requests/${editingRequest._id}`,
          requestData
        );
        if (response.data.success) {
...
Valid Until: ${quoteData.validUntil}
      ...
      const response = await axios.put(
        `http://localhost:5000/api/live-requests/${viewingRequest._id}`,
        {
          status: 'quoted',
      ...
      if (response.data.success) {
        ...
        try {
          const emailResponse = await axios.post(
            `http://localhost:5000/api/live-requests/${viewingRequest._id}/send-quotation-email`,
            {
              quotationData: {
                items: validItems,
